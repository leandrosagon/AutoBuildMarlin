type PaymentIntentLike = { id: string; client_secret?: string; amount: number; currency: string; status: string };
...
export function getStripeClient() {
  if (!stripeSecret) {
    return {
      async createPaymentIntent(amount: number, currency = 'usd'): Promise<PaymentIntentLike> {
  ...
  return {
    async createPaymentIntent(amount: number, currency = 'usd'): Promise<PaymentIntentLike> {
      ...
      return { id: pi.id, client_secret: pi.client_secret, amount: pi.amount, currency: pi.currency, status: pi.status } as PaymentIntentLike;
    }
  };
}
